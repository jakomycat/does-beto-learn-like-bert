...
def main():
    ...
    parser.add_argument( # For quick debugging
        '--max_samples',
        type=int,
        default=None
    )
    ...
    # Get data
    sentences_train_raw = load_premises(split='train', max_samples=args.max_samples)
    sentences_test_raw  = load_premises(split='test', max_samples=args.max_samples)
    ...
    train_tree_roles = preparse_tree_roles(sentences_train, nlp)
    test_tree_roles  = preparse_tree_roles(sentences_test, nlp)
    ...
    targets_train = get_cls_token(sentences_train, model, tokenizer, device, task_name='tpdn', split='train', is_split_into_words=True)
    targets_test  = get_cls_token(sentences_test, model, tokenizer, device, task_name='tpdn', split='test', is_split_into_words=True)
