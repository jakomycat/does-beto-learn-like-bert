...
from src.extractor import load_model_and_tokenizer, get_cls_token, get_filler_token_ids, get_word_embedding_matrix
...
def main():
    ...
    langs, max_samples, max_depth, seeds = args.langs, args.max_samples, args.max_depth, args.seeds
    ...
    for lang in langs:
        ...
        # Get data
        sentences_train_raw = load_premises(split='train', lang=lang, max_samples=max_samples)
        sentences_test_raw  = load_premises(split='test',  lang=lang, max_samples=max_samples)

        sentences_train = [s.split() for s in sentences_train_raw]
        sentences_test  = [s.split() for s in sentences_test_raw]

        # Parse trees and build roles
        stanza.download(lang, processors='tokenize,pos,constituency', verbose=False)
        ...
        # Extract features (FILLERS and CLS tokens)
        filler_ids_train = get_filler_token_ids(sentences_train, tokenizer)
        filler_ids_test  = get_filler_token_ids(sentences_test, tokenizer)

        pretrained_filler_weight = get_word_embedding_matrix(model)
        n_fillers = pretrained_filler_weight.shape[0]
        ...
        # Train and evaluate
        run_tpdn_evaluation(
            filler_ids_train=filler_ids_train,
            filler_ids_test=filler_ids_test,
